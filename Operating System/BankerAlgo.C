...
int main(){
	...
	printf("Enter the process no. that raises a request\n");  //Just give any value when you only run the safety algo.
	scanf("%d", &l);
	printf("Enter the requet matrix for that process\n");     //Just give any value when you only run the safety algo.
	for(int j = 0; j < m; j++)
		scanf("%d", &req[l][j]);
	safety_algo(n, m, alloc, need, avail, f, ans, ind); //This works correctly.
	//req_algo(n, m, alloc, need, avail, req, f, ans, ind);  //This needs improvement as it does not give the correct answer.
	return (0); 
}
...
// 2 2 2
// 4 3 3
// Enter the total no. of available resources
// 3 3 2
// Enter the process no. that raises a request
// 1
// Enter the requet matrix for that process
// 4 5 6
